...
path = kagglehub.dataset_download("baofengz/occluded-duke")
...
def prepare_DukeMTMC_split(root_dir, save_path):
    """
    ...
    ids=[];id_mapping={} #original:new
    def get_id_mapping(dir_path):
        for fname in os.listdir(dir_path):
            ...
            if pid not in id_mapping:
                id_mapping[pid]=len(id_mapping.keys())
    get_id_mapping(train_dir)
    get_id_mapping(query_dir)
    get_id_mapping(gallery_dir)
    print(id_mapping)
    ...
    def process_dir(dir_path):
        ...
        for fname in os.listdir(dir_path):
            ...
            data.append([img_path, id_mapping[pid], cam])
            # ids.append(pid)
    ...
    print(f"DukeMTMC-reID split saved to {save_path}")
    print('total:',len(set(ids)),max(ids),min(ids))
...
DukeMTMC_root = path
    # Set the output JSON file path (for example, 'DukeMTMC_split.json')
output_json = 'DukeMTMC-reID_split.json'
id_mapping=prepare_DukeMTMC_split(DukeMTMC_root, output_json)
...
class Classifier(nn.Module):
    def __init__(self,in_features,num_classes=1812):
...
def main(train=False, epochs=1, json_file='market1501_split.json', model=None, device=None, lr=1e-3):
    ...
    if train:
        dataloader = get_dataloader(train_data, batch_size=8,num_triplets=12000)
...
main(train=True, epochs=5, json_file='DukeMTMC-reID_split.json', model=model, device=device,lr=1e-5)
# -------------------
#torch.save(model.state_dict(), 'market1501_model_weights.pth')
#torch.cuda.empty_cache()