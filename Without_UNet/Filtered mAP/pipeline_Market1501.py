...
def re_ranking(query_cam, all_dist, k1=20, k2=6, lambda_value=0.3):
    """
    Re-ranking function using k-reciprocal nearest neighbors (k-NN).
    Args:
    - query_cam: The camera IDs for the queries.
    - all_dist: The distance matrix between queries and galleries.
    - k1, k2: The number of neighbors to consider for re-ranking.
    - lambda_value: A weight for combining the original distance and re-ranked distance.

    Returns:
    - re_ranked_dist: The re-ranked distance matrix.
    """
    # Get the number of queries and gallery images
    num_queries, num_gallery = all_dist.shape

    # Initialize re-ranked distances
    re_ranked_dist = np.zeros_like(all_dist)

    for i in range(num_queries):
        # Original distances for the i-th query
        dist = all_dist[i]

        # Find the k1 nearest neighbors
        neighbors = np.argsort(dist)[:k1]

        # Perform the reciprocal re-ranking
        reciprocal_neighbors = np.argsort(all_dist[neighbors], axis=1)[:, :k2]
        re_ranked_dist[i] = np.mean(all_dist[reciprocal_neighbors], axis=1)

        # Combine the original and re-ranked distances
        re_ranked_dist[i] = lambda_value * re_ranked_dist[i] + (1 - lambda_value) * dist

    return re_ranked_dist

def apply_reranking(all_dist, query_cam, k1=20, k2=6, lambda_value=0.3):
    """
    Apply re-ranking to the distance matrix.
    Args:
    - all_dist: The initial distance matrix (query-gallery).
    - query_cam: Camera IDs for the queries.
    - k1, k2: The parameters for k-reciprocal re-ranking.
    - lambda_value: Weight for the original vs. re-ranked distance.

    Returns:
    - re_ranked_dist: The re-ranked distance matrix.
    """
    # Apply re-ranking
    re_ranked_dist = re_ranking(query_cam, all_dist, k1, k2, lambda_value)
    return re_ranked_dist

# After computing the initial distance matrix `all_dist`
# print('Applying re-ranking...')
# re_ranked_dist = apply_reranking(all_dist, query_cam)

# # Now, continue with the evaluation using the re-ranked distance
# CMC, mAP = evaluate_rank_map(torch.tensor(re_ranked_dist), query_label, gallery_label, query_cam, gallery_cam)

# print(f"Re-ranked CMC: {CMC}")
# print(f"Re-ranked mAP: {mAP}")




def evaluate_rank_map(euclidean_dist, query_labels, gallery_labels, query_cams, gallery_cams):
    """
    Computes Cumulative Matching Characteristic (CMC) and mean Average Precision (mAP),
    while excluding same-ID same-camera matches (as per standard ReID protocol).
    """
    ...
    for i in range(num_query):
        ...
        indices = np.argsort(dist)  # Lower distance = higher similarity

        # Get labels and cams of sorted gallery entries
        sorted_labels = gallery_labels[indices]
        sorted_cams = gallery_cams[indices]

        # Determine good and junk indices
        # good: same ID, different cam
        # junk: same ID, same cam or label == -1
        mask_good = (sorted_labels == q_label) & (sorted_cams != q_cam)
        mask_junk = ((sorted_labels == q_label) & (sorted_cams == q_cam)) | (sorted_labels == -1)

        if not np.any(mask_good):  # No good match, skip
            continue

        # Remove junk entries
        valid = ~mask_junk
        filtered_matches = (sorted_labels[valid] == q_label).astype(int)

        # CMC update
        rank_idx = np.where(filtered_matches == 1)[0][0]
        CMC[rank_idx:] += 1

        # AP calculation
        num_rel = filtered_matches.sum()
        tmp_cmc = np.cumsum(filtered_matches) / (np.arange(len(filtered_matches)) + 1)
        AP = (tmp_cmc * filtered_matches).sum() / num_rel
        APs.append(AP)
...
def main(train=False, epochs=1, json_file='market1501_split.json', model=None, device=None, lr=1e-3):
    ...
    if train:
        ...
        for e in range(epochs):
            ...
            model.eval()
            query_features, query_labels,query_cams = extract_features(model, query_loader, device)
            gallery_features, gallery_labels,gallery_cams = extract_features(model, gallery_loader, device)
            euclidean_dist = compute_euclidean_distance(query_features, gallery_features)
            euclidean_dist= apply_reranking(euclidean_dist,query_cams)
            CMC, mAP = evaluate_rank_map(euclidean_dist, query_labels, gallery_labels,query_cams,gallery_cams)
...
model.load_state_dict(checkpoint['siamese_clip_state'])
classifier.load_state_dict(checkpoint['classifier_state'])
main(train=False, epochs=5, json_file='market1501_split.json', model=model, device=device,lr=1e-5)
# -------------------
#torch.save(model.state_dict(), 'market1501_model_weights.pth')
#torch.cuda.empty_cache()